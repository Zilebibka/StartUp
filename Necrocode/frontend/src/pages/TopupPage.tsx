...
export function TopupPage({ balance, topupAmount, setTopupAmount, paymentMethod, setPaymentMethod, handleTopup }: TopupPageProps) {
  return (
    <motion.section key="topup" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} className="max-w-3xl mx-auto bg-white border rounded-2xl shadow-sm p-4 sm:p-8 space-y-5 sm:space-y-6 panel-shell">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 min-[420px]:grid-cols-3 sm:flex sm:flex-wrap gap-2">
          {[100, 500, 1000, 2000, 5000].map((amount) => (
            <button key={amount} onClick={() => setTopupAmount(amount)} className={`px-3 sm:px-4 py-2 border-2 rounded-xl text-sm panel-button ${topupAmount === amount ? 'bg-black text-white panel-button-active' : ''}`}>{amount} ₽</button>
          ))}
        </div>
        <input type="number" value={topupAmount} onChange={(e) => setTopupAmount(e.target.value ? Number(e.target.value) : '')} className="w-full px-4 py-3 bg-gray-50 border rounded-xl panel-soft panel-outline" min="100" />
      </div>
